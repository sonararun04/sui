...
impl SuiNode {
    ...
    pub async fn monitor_reconfiguration(self: Arc<Self>) -> Result<()> {
        ...
        loop {
            ...
            #[cfg(msim)]
            let cur_protocol_version = cur_epoch_store.epoch_start_state().protocol_version();
            ...
            if !self.sim_safe_mode_expected.load(Ordering::Relaxed)
                // It is known that we have a bug in older protocol version that could lead to safe mode.
                && ProtocolConfig::get_for_version(cur_protocol_version).get_advance_epoch_start_time_in_safe_mode()
            {
